...
1. `main` 函数是 `src/main.rs` 文件中程序的起点，程序从这里开始执行。

## cargo 常用命令

cargo 是 Rust 的构建工具和包管理器(类似应用商店)，提供了丰富的命令来帮助开发者管理项目、依赖和构建流程。以下是一些常用的 cargo 命令：
...
`cargo test` 命令用于运行项目中的测试代码，支持多种参数来控制测试的行为,在[自动化测试](16.编写自动化测试.md)章节中有详细介绍。
