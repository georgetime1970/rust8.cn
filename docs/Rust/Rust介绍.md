---
description: Rust 是一门注重内存安全与高性能的系统级编程语言,广泛应用于操作系统、网络服务、WebAssembly、区块链等领域,拥有活跃的生态与社区.
---
...
- **内存安全**: 通过所有权(Ownership)系统在编译期检查内存使用,无需 GC,无[空指针](0.基础概念.md#空指针)、无[悬垂指针](0.基础概念.md#悬垂指针),最大化避免内存错误.
- **高性能**: 与 C/C++ 处于同一性能量级,[零成本抽象(Zero-cost Abstraction)](0.基础概念.md#零成本抽象)
- **并发安全**: 类型系统和所有权规则从根本上防止了多线程[数据竞争](./无畏并发延申.md#数据竞争)
