...
- `str` 类型是**字符串数据本体**,也是 `String` 类型的**切片类型**,通常以引用形式 `&str` 出现.
- `String` 类型类似 `str` 类型的容器,是由标准库提供,封装了 `Vec<u8>` 的结构体,支持可增长、可变、可拥有的 UTF-8 编码字符串.
...
使用一种简化的也不算严谨的方式来描述他们的关系,便于你理解:

Rust 需要一种方式来表示"一串连续的文本",于是使用了 `[u8]` 数组.

但是 `[u8]` 数组可能是任何字节序列,并不一定是有效的 UTF-8 编码.为了确保字符串数据的有效性,Rust 定义了 `str` 类型,它是一个针对 `[u8]` 字节数组的类型安全包装（Type Wrapper）,保证了数据的正确性和安全性.

所以 在逻辑层面上，`str` 本质上就是一种“规则”和“类型抽象”

`str` 代表一段连续的 UTF-8 字节序列,是**字符串数据的本体**.
...
对一个 `String` 类型做切片,得到的就是 `str` 类型；`str` 就是字符串字面量的实际内容.Rust 中所有字符串的数据本质上都是 `str`.

> 推荐先阅读 [String 字符串延申](./String延申.md) 章节,深入理解 `str`、`&str` 和 `String` 的关系,再回来看这个章节会更有收获.

## 字符串类型

字符串类型主要有两种使用形式: [字符串字面量 &str](#字符串字面量-str) 和 [String 类型](#string-类型).
...
以下方法定义在 `str` 上.由于 `String` 实现了 `Deref<Target = str>`,`&str` 和 `String` 均可直接调用.
...
// 字符串转其他类型
// 需要实现 FromStr trait 的类型才能使用 parse() 转换
...
> `len()` 返回字节数；含多字节字符时用 `chars().count()` 获取字符数:
>
> ```rust
> let s = "你好";
