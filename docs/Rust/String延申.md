...
`str` 代表一段连续的 UTF-8 字节序列，是**字符串数据的本体**。对一个 `String` 做切片，得到的就是 `str`；字符串字面量的实际内容，也是 `str`。Rust 中所有字符串的数据本质上都是 `str`。
...
> 可以把 `str` 理解成一段录音的内容本身。`String` 是你自己的录音机，买来就归你，可以追加录音；`&str` 是借别人的录音机来听，只能听，不能改。
