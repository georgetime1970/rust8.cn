---
description: 本文介绍 Rust 的异步编程，包括 async/await 语法、Future、Stream 及其在高并发和 I/O 密集型场景下的应用。
---
...
`trpl` 库常用于《The Rust Programming Language》配套示例,便于学习 async 概念.

- `trpl::block_on()`: 阻塞当前线程直到 future 完成(多线程运行时).
- `trpl::block_on()` 只能等待一个 future；若需等待多个,可结合 `trpl::join(<future1>, <future2>)` 或 `trpl::join!(<future1>, <future2>, ...)` 使用.
...
// 上面 trpl::spawn_task 并发上等同于下面 trpl::join,但底层机制不同
trpl::block_on(async {
...
Rust 为 `async` 代码块生成的 future 可能包含**自引用**(某字段引用自身其他字段).若 future 在执行过程中被移动,这些指针就会变成[悬垂指针](./0.基础概念.md#悬垂指针),导致内存安全问题.
