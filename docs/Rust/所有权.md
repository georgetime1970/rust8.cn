...
- **新版编译器([非词法作用域生命周期 NLL](./作用域#非词法作用域生命周期))**: 引用的作用域从定义到**最后一次使用**的位置
...
**本质理解**: 可变引用的排他性针对的是**同一时刻是否同时使用**,而非"是否在同一作用域内定义".Rust 编译器借助 [NLL](./作用域#非词法作用域生命周期) 精确追踪每个引用的最后使用位置,只要不在同一时刻使用多个可变引用,就是合法的.
