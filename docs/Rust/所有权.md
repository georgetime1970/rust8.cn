---
description: 本文详细介绍了 Rust 语言的所有权(Ownership)机制，包括所有权的三大规则、所有权的转移(Move)、借用(Borrow)、不可变与可变引用、作用域与生命周期等内容。通过丰富的代码示例和深入浅出的讲解，帮助读者理解 Rust 如何通过所有权系统实现内存安全和高性能，无需垃圾回收。
---
...
- 每个值在任意时刻有且只能有**一个所有者**
- 当所有者(变量)离开作用域时，这个值将被**销毁**([调用 `drop`](./0.基础概念.md#析构函数) 并回收内存)

> **核心特性**: Rust 通过编译器在**语义层面**强制执行所有权规则，保证内存安全，避免了[垃圾回收](./0.基础概念.md#垃圾回收器)开销和[野指针](./0.基础概念.md#悬垂指针)问题，这是 Rust 的核心设计理念之一。
...
`String` 类型在**栈**上存储的是一个[超胖指针](./内存和指针.md#瘦指针与胖指针)(包含指针、长度、容量三个字段)，实际字符串数据存储在**堆**上。变量 `s` 绑定的是栈上的那个超胖指针，因此 `s` 是**超胖指针的所有者**，而不是堆中实际数据的所有者。

不过，由于超胖指针指向堆中数据，为了简化描述，通常也会说"s 是那段字符串数据的所有者"。
...
println!("{}", s);       //[!code error] ❌ 编译错误: s 已失效
println!("{}", s2);      // ✅ 输出 "hello"
```
...
b.push_str("!");          //[!code error] ❌ 编译错误: b 是不可变的
println!("{}", b);
```
...
对于 `Vec<T>`、`String` 这类复杂类型，即使发生了 Move，拷贝的也只是栈上的超胖指针部分(24 字节)，而不是整个数据。Rust 编译器也会对 Move 语义做进一步优化，在适当情况下直接传递指针而非内存拷贝。
...
```rust
let v = &vec![1, 2, 3];
let vv = *v;  //[!code error] ❌ 错误: cannot move out of `*v` which is behind a shared reference
```
...
println!("{}", x);    //[!code error] ❌ 错误: value borrowed here after move
```
...
println!("{}", y);    // ✅
println!("{}", x);    //[!code error] ❌ 错误: x 已被移走
```
...
println!("{}", tup.0);       // ✅ 打印: 5, tup.0 是 i32,实现了 Copy
println!("{:?}", tup);       //[!code error] ❌ tup 不完整,不可使用
println!("{}", tup.1);       //[!code error] ❌ tup.1 的所有权已被转走
```
...
// 旧写法
let (x, ref y) = tup;    // 通过 ref 创建 y 的引用,解构时 y 是引用类型
println!("{}", tup.1);   // ✅ y 是借用,没有发生 Move

// 新写法
let (x, y) = &tup;       // 直接对 tup 创建引用,解构时 y 是引用类型
println!("{}", tup.1);   // ✅ tup 没有被移动,仍可访问
```
...
// ❌ 错误: 不能加在变量名前面
fn execute<T>(&item: T) { ... } // [!code error]
```

## 借用

**借用(Borrow)** 是通过引用访问数据而不获取所有权的**行为**。创建引用即是在借用。
...
let r1 = &s;              // ✅ 不可变引用
let r2 = &s;              // ✅ 多个不可变引用可以共存
let r3 = &mut s;          //[!code error] ❌ 不能在有不可变引用的同时创建可变引用
println!("{}, {}", r1, r2);
...
> **设计原因**: 借用规则从根本上防止了[数据竞争(data race)](./无畏并发延申.md#数据竞争)。Rust 在编译阶段就杜绝了这种情况。

### 引用的作用域(NLL)

引用的作用域与普通变量有所不同:

- **旧版编译器(Rust 1.31 之前)**: 引用的作用域从定义到所在花括号`}`结束
- **新版编译器([非词法作用域生命周期 NLL](./作用域.md#非词法作用域生命周期))**: 引用的作用域从定义到**最后一次使用**的位置
...
println!("{}", r1); //[!code error] ❌ r1 和 r2 在此同时有效,编译错误
println!("{}", r2);
```
...
**本质理解**: 可变引用的排他性针对的是**同一时刻是否同时使用**，而非"是否在同一作用域内定义"。Rust 编译器借助 [NLL](./作用域.md#非词法作用域生命周期) 精确追踪每个引用的最后使用位置，只要不在同一时刻使用多个可变引用，就是合法的。
...
> println!("{}"， r1);     //[!code error] ❌ r1 已失效
> println!("{}"， x);      // 原变量 x 抢占独占锁，r2 失效
> println!("{}"， r2);     //[!code error] ❌ r2 已失效
> ```
>
...
> **自动解引用**: 调用方法时(`.` 操作符)，编译器会自动处理解引用，无需显式 `*`。只有在手动修改值或数值运算等场景下才需要显式解引用。背后是通过实现 [Deref Trait](./智能指针.md#deref-trait-解引用运算符) 来实现的。
...
*r1 = String::from("hello"); //[!code error] ❌ r2 存在时,r1 不可使用(借用规则)
println!("{}", s);           // 输出 "world"
```
