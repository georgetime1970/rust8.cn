...
Rust 的控制流主要分为 [条件判断](#条件判断) 和 [循环](#循环) 两大类，它们允许你根据不同的条件执行不同的代码块，或者重复执行某段代码直到满足特定条件。
...
`if` 的各个分支可能产生的结果值都必须是**相同类型**;否则编译器会报错。
...
Rust 有三种循环: [loop](#loop-循环)、[while](#while-循环) 和 [for](#for-循环) 循环。
