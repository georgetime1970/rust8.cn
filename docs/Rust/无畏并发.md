---
description: 本文介绍 Rust 的无畏并发(Fearless Concurrency)特性，讲解如何利用 Rust 编译器的检查机制安全地进行多线程编程，避免数据竞争和死锁等并发 bug，并通过示例演示线程的创建与管理。
---
...
fn main() {
    let (tx, rx) = mpsc::channel();    // 创建信道,返回发送端和接收端

    thread::spawn(move || {
        let val = String::from("hi");
        // 发送数据到信道,如果信道已关闭则返回错误,会转移所有权
        tx.send(val).unwrap();
    });

    // // 从信道接收数据,如果当前没有数据则阻塞等待
...
fn main() {
  let (tx, rx) = mpsc::channel();  // 创建信道
    let tx1 = tx.clone();          // 克隆发送端,创建第二个发送端
    thread::spawn(move || {
        let vals = vec![
            String::from("hi"),
            String::from("from"),
        ...
        for val in vals {
            tx1.send(val).unwrap(); // 使用克隆的发送端发送数据
    ...
    thread::spawn(move || {
        ...
        for val in vals {
            tx.send(val).unwrap();  // 使用原始发送端发送数据
            thread::sleep(Duration::from_secs(1));
        }
    });
...
fn main() {
    let m = Mutex::new(5);               // 创建一个互斥锁,并初始化值为 5
    {
        let mut num = m.lock().unwrap(); // 获取互斥锁,返回 MutexGuard 智能指针
        *num = 6;                        // 修改被保护的数据
    }                                    // num 离开作用域,自动释放互斥锁

    {
        let mut num = m.lock().unwrap(); // 再次获取互斥锁,返回 MutexGuard 智能指针
        *num = 6;                        // 修改被保护的数据
    }                                    // 这里 num 离开作用域,自动释放锁
...
std::thread::spawn(move || {
    // 这里 x 被"搬到"新线程了,所有权转移给了新线程
    // x 实现了Send trait,所以可以在线程间转移
    println!("x = {x}");
});
```
...
fn main() {
    ...
    for i in 0..3 {
        // Vec<T>实现了Sync trait,所以引用可以在线程间共享
        let data = Arc::clone(&data);
...
> `Rc<T>` 不实现 `Send` 和 `Sync`；`RefCell<T>` 不实现 `Sync`（但当 `T: Send` 时实现 `Send`）
