---
description: 介绍 Rust 智能指针的原理、特性与常见用法,包括 Box、Rc、Deref、Drop 等内容.
---
...
禁止直接调用 `.drop()` 方法(编译器会阻止,因为这会导致[二次释放(double free)](./0.基础概念.md#二次释放)).若需提前手动释放,使用标准库的 `drop(value)` 函数:
...
一个类型不能同时实现 `Copy` 和 `Drop`.`Copy` 类型会被隐式复制,难以预测[析构函数](./0.基础概念.md#析构函数)执行的时机,因此 Rust 明确禁止:
