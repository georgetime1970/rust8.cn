...
泛型默认可以代表任意类型，但函数体内往往需要对参数执行特定操作(如加法、比较、打印)，这要求泛型所代表的类型必须具备相应的能力([Trait](./Trait特征.md))，这就是泛型函数的 **Trait 约束**(Trait Bound)
...
```rust
// ❌ 不推荐: 在 struct 定义时就限制
struct Food<T: Debug>(T); // [!code error] 过度限制,所有 Food<T> 的 T 都必须实现 Debug Trait
