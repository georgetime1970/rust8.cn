...
函数名后的 `<T>` 在函数作用域内**声明**一个泛型 `T`，声明后才能在函数签名和函数体中使用它; `(v: T)` 参数类型是 `T`; `-> T` 表示返回值类型是 `T`:
...
泛型默认可以代表任意类型，但函数体内往往需要对参数执行特定操作(如加法、比较、打印)，这要求泛型所代表的类型必须具备相应的能力([Trait](./Trait特征.md))，这就是 **Trait 约束**(Trait Bound)
