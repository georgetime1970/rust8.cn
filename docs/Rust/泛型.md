---
description: 本文介绍 Rust 语言中的泛型(Generic)，包括其定义、作用、使用方法及相关示例，帮助读者理解如何通过泛型减少代码冗余并提升代码复用性。
---

# 泛型

变量是对内存中**数据**的[抽象](./0.基础概念.md#抽象): 在编写代码阶段可以用变量名代表某个数据，编译阶段再替换为具体的内存地址。

泛型(Generic)是对**数据类型**的抽象，与变量的含义相同，在编写代码阶段可以用泛型代表各种可能的数据类型，编译阶段再替换为具体的类型。
