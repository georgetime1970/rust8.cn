---
description: 本文介绍如何在 Rust 项目中编写自动化测试,包括测试函数的结构、断言的使用方法,以及如何通过测试提升代码的正确性和健壮性.
---
...
> 文档测试**默认用于库项目(lib)**,二进制项目(`main.rs`)需要使用 `cargo doc --bin 项目名` 生成文档.
...
> 在 main.rs 中的函数通常是私有的，但无需 --document-private-items 参数即可在文档中查看；
>
> 而在 lib.rs 中的私有函数，必须使用 --document-private-items 参数才能查看文档。
