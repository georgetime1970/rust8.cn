...
迭代器是 Rust 中处理序列数据的核心抽象.与 C 风格的 `for (i = 0; i < len; i++)` 循环不同,Rust 的迭代器不需要手动管理索引、边界检查或循环状态——你只需描述"对每个元素做什么",而不是"怎么遍历".这种抽象不仅让代码更简洁,而且由于[**零成本抽象**](./0.基础概念.md#零成本抽象)原则,迭代器在运行时与等价的手写循环效率相同甚至更高.
