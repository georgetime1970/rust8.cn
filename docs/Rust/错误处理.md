---
description: 介绍 Rust 语言中的错误处理机制，包括 panic! 和 Result 的用法。
---
...
> `Option<T>` 详细介绍请查看 [Option枚举](./Option枚举.md) 一章
...
| 方法          | 作用                                                      |
| ------------- | --------------------------------------------------------- |
| `and(res)`    | `Ok` 时返回 `res`；`Err` 时透传自身错误（`res` 立即求值） |
| `and_then(f)` | `Ok(v)` 时调用 `f(v)`（`f` 返回 `Result`）；`Err` 时短路  |
| `or(res)`     | `Err` 时返回 `res`；`Ok` 时透传自身值（`res` 立即求值）   |
...
| 链式调用中插入日志   | `.inspect` / `.inspect_err` | 调试时查看中间值，不打断链式      |
