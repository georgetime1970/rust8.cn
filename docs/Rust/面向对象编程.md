---
description: 本文介绍 Rust 语言中的面向对象编程特性，包括对象、封装、继承与多态等内容，并结合实际代码示例，帮助读者理解 Rust 如何实现面向对象思想。
---
...
> 详见 [泛型](./泛型.md#trait-约束-trait-bound) 章节.
...
> 详见 [Trait 特征](./Trait特征.md#Trait-对象-dyn-trait)
...
// 使用示例:
let mut post = DraftPost::new();   // 创建草稿状态的博文
post.add_text("今天天气很好.");     // 添加文本到草稿博文
let post = post.request_review();  // 请求审核,转换为待审核状态
let post = post.approve();         // 审核通过,转换为已发布状态
println!("{}", post.content());    // 输出已发布博文内容
```
