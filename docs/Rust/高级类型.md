---
description: 介绍 Rust 的高级类型特性，包括类型别名(type)、Never 类型(!)、动态大小类型(DST)与 Sized trait 的用法和原理。
---
...
```rust
let s1: str = "Hello there!";      // 12 字节
let s2: str = "How's it going?";   //[!code error] ❌ 错误:15 字节,与s1同类型但大小不同
```

这两个变量都是 `str` 类型，编译器要求它们必须占用相同的空间，但它们的大小不同，因此编译器无法通过编译，所以不能直接创建 `str` 类型的变量。
